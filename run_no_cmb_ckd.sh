#!/usr/bin/env bash
# Run the AKI prediction pipeline excluding cmb_ckd==1 and renal_impaired_at_adm==1 patients upstream.
# Outputs to features_no_cmb_ckd/ and artifacts_no_cmb_ckd/ — does NOT overwrite existing runs.
#
# Run from project root:
#   bash run_no_cmb_ckd.sh [--skip-features] [--skip-imputation] [--skip-training] [--device=cpu|cuda]
...
FEAT_DIR="features_no_cmb_ckd"
ART_DIR="artifacts_no_cmb_ckd"
TARGET="target_no_cmb_ckd.parquet"

log() { echo; echo "==> $*"; }

if [ ! -f "$TARGET" ]; then
  echo "ERROR: $TARGET not found. Run the exclusion step first."
  exit 1
fi
